...
<!-- Sidebar end=============================================== -->
	...
	<?php 
	  ...
	  if ($_SESSION['customer_id']) {
         ...
         $selectQuery="SELECT * FROM `table_order` WHERE customer_id='$customer_id' AND status='0' ";
         $runseQuery=mysqli_query($connect,$selectQuery);
         if($runseQuery){
         	$sum=0;
         	while ($result=mysqli_fetch_assoc($runseQuery)) {
         		
         		$product_quenty=$result['product_quenty'];
         		$reprice=$result['price']*$product_quenty;
         		$sum=$sum+$reprice;
         	}
         }
	 ...
	 <div class="order_detelis">
	 	<h1>
	 	<?php 
	 		$discount=$sum/100*15;

	 		echo "- Discount 15% :$discount TK";
	 	...
	 	<h1>
	 		...
	 		$discountali=$discount-$vat;

	 		$sumAl=$sum-$discountali;
	 	...
	 	<h1>Total payable Amount :<?php echo $sumAl; ?> TK</h1>
	 	<p>Thanks For purchase receive your order successfull .we will contact you ASAP with details .here is your order details <a class="btn btn-success" href="order_listShow.php?customer_id=<?php echo $customer_id; ?>">Details</a>	</p>
